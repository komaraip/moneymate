...
<div class="p-6">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2">Card Types</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Card Types</h1>
            <p class="mt-1 text-sm text-gray-600">Manage card types like Visa, Mastercard, and GPN</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <dt class="text-sm font-medium text-gray-500">Total Card Types</dt>
                    <dd class="text-2xl font-bold text-gray-900">{{ $cardTypes->total() }}</dd>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <dt class="text-sm font-medium text-gray-500">Active Types</dt>
                    <dd class="text-2xl font-bold text-gray-900">{{ $cardTypes->where('is_active', true)->count() }}</dd>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <dd class="text-2xl font-bold text-gray-900">{{ $cardTypes->sum('users_count') ?? 0 }}</dd>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center">
                <div class="flex-shrink-0">
                    <svg class="w-8 h-8 text-purple-600" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M19 4h-1a1 1 0 1 0 0 2v11a1 1 0 0 1-2 0V2a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v15a1 1 0 0 1-2 0V6a1 1 0 0 0 0-2H1a1 1 0 0 0 0 2v11a3 3 0 0 0 3 3h12a3 3 0 0 0 3-3V6a1 1 0 1 0 0-2ZM10 6a1 1 0 0 1 1 1v3a1 1 0 1 1-2 0V7a1 1 0 0 1 1-1Zm0 8a1 1 0 1 1 0-2 1 1 0 0 1 0 2Z"/>
                    </svg>
                </div>
                <div class="ml-4">
                    <dt class="text-sm font-medium text-gray-500">With Logo</dt>
                    <dd class="text-2xl font-bold text-gray-900">{{ $cardTypes->whereNotNull('logo_url')->count() }}</dd>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow mb-6">
        <div class="p-4">
            <form method="GET" action="{{ route('admin.card-types.index') }}" class="flex flex-col sm:flex-row gap-4">
                <div class="flex-1">
                    <input type="text"
                           name="search"
                           value="{{ request('search') }}"
                           placeholder="Search card types by name..."
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($cardTypes as $cardType)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    @if($cardType->logo_url)
                                        <img class="h-8 w-12 mr-3 object-contain rounded" src="{{ $cardType->logo_url }}" alt="{{ $cardType->name }} logo">
                                    @else
                                        <div class="h-8 w-12 mr-3 bg-gray-200 rounded flex items-center justify-center">
                                    ...
                                    @endif
                                    <div>
                                        <div class="text-sm font-medium text-gray-900">{{ $cardType->name }}</div>
                                        @if($cardType->description)
                                            <div class="text-sm text-gray-500">{{ Str::limit($cardType->description, 50) }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($cardType->brand_color)
                                    <div class="flex items-center space-x-2">
                                        <div class="w-4 h-4 rounded border" style="background-color: {{ $cardType->brand_color }}"></div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $cardType->users_count ?? 0 }} users
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-2">
                                    <a href="{{ route('admin.card-types.show', $cardType) }}"
                                    ...
                                    <form action="{{ route('admin.card-types.destroy', $cardType) }}"
                                          method="POST"
                                          class="inline"
                                          onsubmit="return confirm('Are you sure you want to delete this card type?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <svg class="mx-auto h-12 w-12 text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2H4Zm0 2h12v2H4V6Zm0 4h12v4H4v-4Z"/>
                                </svg>
                                <h3 class="mt-2 text-sm font-medium text-gray-900">No card types found</h3>
                                <p class="mt-1 text-sm text-gray-500">Get started by creating a new card type.</p>
                                <div class="mt-6">
                                    <a href="{{ route('admin.card-types.create') }}"
