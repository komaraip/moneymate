...
<div class="p-6">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2">Edit Card Type</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Card Type</h1>
            <p class="mt-1 text-sm text-gray-600">Update card type information</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden">
        <form action="{{ route('admin.card-types.update', $cardType) }}" method="POST" class="divide-y divide-gray-200">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <textarea id="description"
                                  ...
                                  placeholder="Brief description of the card type">{{ old('description', $cardType->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text"
                               id="brand_name"
                               name="brand_name"
                               value="{{ old('brand_name', $cardType->brand_name) }}"
                    ...
                    <div>
                        ...
                        <div class="flex space-x-2">
                            <input type="color"
                                   id="brand_color"
                                   name="brand_color"
                                   value="{{ old('brand_color', $cardType->brand_color ?? '#1f2937') }}"
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <input type="url"
                               id="logo_url"
                               name="logo_url"
                               value="{{ old('logo_url', $cardType->logo_url) }}"
                        ...
                        @if($cardType->logo_url)
                            <div class="mt-2">
                                <p class="text-sm text-gray-700 mb-1">Current logo:</p>
                                <img src="{{ $cardType->logo_url }}" alt="{{ $cardType->name }} logo" class="h-12 w-18 object-contain rounded border">
                            </div>
                        @endif
                    </div>
            ...
            <div class="px-6 py-6 bg-gray-50">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="bg-white rounded-lg p-4 border">
                        ...
                        <dd class="text-2xl font-bold text-gray-900">{{ $cardType->users_count ?? 0 }}</dd>
                    </div>
                    <div class="bg-white rounded-lg p-4 border">
                        <dt class="text-sm font-medium text-gray-500">Created</dt>
                        <dd class="text-sm text-gray-900">{{ $cardType->created_at->format('M j, Y') }}</dd>
                    </div>
                    <div class="bg-white rounded-lg p-4 border">
                        <dt class="text-sm font-medium text-gray-500">Last Updated</dt>
                        <dd class="text-sm text-gray-900">{{ $cardType->updated_at->format('M j, Y') }}</dd>
                    </div>
                </div>
                ...
                @if($cardType->users_count > 0)
                    <div class="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                <h3 class="text-sm font-medium text-blue-800">Usage Notice</h3>
                                <p class="mt-1 text-sm text-blue-700">
                                    This card type is currently being used by {{ $cardType->users_count }} user(s).
                                    Disabling this card type may affect these users' profiles.
                                </p>
                            </div>
                        </div>
                    </div>
