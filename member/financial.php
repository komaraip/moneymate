<?php
    ...
    // Pastikan email tersedia di session setelah proses login
    if (isset($_SESSION['email'])) {
        ...
        if ($stmt_user->execute()) {
            ...
            if ($result_user->num_rows > 0) {
                ...
                $userID = $row_user['id']; // Ambil userID dari hasil query user
    
                // Simpan informasi user ke dalam session
                $_SESSION['id'] = $userID;
                $_SESSION['fullname'] = $row_user['fullname'];
                $_SESSION['email'] = $row_user['email'];
                $_SESSION['balance'] = $row_user['balance'];
                $_SESSION['cardnumber'] = $row_user['cardnumber'];
                $_SESSION['card'] = $row_user['card'];
                $_SESSION['currency'] = $row_user['currency'];
                ...
                if ($stmt_trans->execute()) {
                    ...
                    echo "Error: Failed to execute transaction query.";
                }
                
                $sql_spend = "SELECT category, SUM(balance) AS total_balance
                          FROM trans
                          WHERE userID = ?
                          GROUP BY category";
                
                // Persiapkan statement SQL
                $stmt_spend = $conn->prepare($sql_spend);
                
                if ($stmt_spend) {
                    // Bind parameter dan eksekusi statement
                    $stmt_spend->bind_param("i", $userID);
                    $stmt_spend->execute();
            ...
            echo "Error: Failed to execute user query.";
        }
						...
						<div class="card box-shadow-none">
							<div class="card-header d-block d-sm-flex border-0">
								<div class="me-3">
									...
									<p>Your previous transactions</p>
								</div>
							</div>
                                        ...
                                        <table class="table table-responsive-md card-table transactions-table">
                                            <tbody>
                                                <tr>
                                                  ...
                                                  <tr>
                                                    <td><?php echo $i++; ?></td>
                                                    <td><?php echo htmlspecialchars($row_trans["transaction"]); ?></td>
                                                    <td><?php echo htmlspecialchars($row_trans["date"]); ?></td>
                                                    <td><?php echo htmlspecialchars($row_trans["time"]); ?></td>
                                                    <td><?php echo htmlspecialchars($row_trans["balance"]); ?></td>
                                                    <td><?php echo htmlspecialchars($row_trans["category"]); ?></td>
                                                  </tr>
							...
							<div class="card-footer border-0 pt-0">
							    <a href="update-5.php" class="btn btn-primary d-block btn-lg">Update</a>
							</div>
						</div>
					</div>
						...
						<div class="card box-shadow-none">
							<div class="card-header d-block d-sm-flex border-0">
								<div class="me-3">
									...
									<p>Your transactions summary by category</p>
								</div>
							</div>
                                        ...
                                        <table class="table table-responsive-md card-table transactions-table">
                                            <tbody>
                                                <tr>
                                                  ...
                                                  <?php
                                                    if (isset($result_spend)) {
                                                        $i = 1;
                                                        while ($row = $result_spend->fetch_assoc()) {
                                                  ?>
							...
							<div class="card-footer border-0 pt-0">
								<h4 class="btn d-block btn-lg">Remember Your Allowance Limit</h4>
							</div>
						</div>
					</div>
				</div>
