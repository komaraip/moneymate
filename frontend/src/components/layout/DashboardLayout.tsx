...
export function DashboardLayout() {
  ...
  return (
    <div className="min-h-screen bg-app text-main">
      <aside className="fixed inset-y-0 left-0 hidden w-72 overflow-y-auto border-r border-subtle bg-app px-5 py-6 lg:block">
        <SidebarContent />
      </aside>
      ...
      {mobileNavOpen ? (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <aside
            ...
            className="relative z-10 flex h-full w-[min(20rem,85vw)] flex-col overflow-y-auto border-r border-subtle bg-app px-5 py-6 shadow-2xl"
          ...
          >
            <div className="mb-4 flex items-start justify-between gap-4">
              ...
              <button
                aria-label="Tutup navigasi"
                className="rounded-lg border border-subtle p-2 text-muted"
                onClick={() => setMobileNavOpen(false)}
                type="button"
              >
                <X className="h-5 w-5" />
      ...
      <main className="lg:pl-72">
        <header className="sticky top-0 z-10 border-b border-subtle bg-app/90 px-5 py-4 backdrop-blur lg:px-8">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <button
                ...
                className="rounded-lg border border-subtle p-2 text-muted lg:hidden"
              ...
              <div>
                <p className="text-sm text-muted">Data bukan real-time</p>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                className="inline-flex items-center gap-2 rounded-lg border border-subtle px-3 py-2 text-sm text-muted hover:border-danger hover:text-danger"
                onClick={() => void logout()}
                type="button"
              >
        ...
        <footer className="px-5 pb-6 text-xs leading-5 text-muted lg:px-8">
...
function Brand() {
  return (
    <div>
      ...
      <h1 className="mt-2 text-xl font-semibold text-main">
        Personal Finance
      </h1>
    </div>
  );
    ...
    <nav className="mt-8 space-y-6">
      {groups.map((group) => (
        <section aria-labelledby={`nav-${group.title}`} key={group.title}>
          <p
            className="px-3 text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-muted"
            id={`nav-${group.title}`}
          >
            {group.title}
          </p>
